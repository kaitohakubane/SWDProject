...
public class SQLParamConstant {
    ...
    public static final String GET_ALL_BILL_BY_DAY = "SELECT billId, date, totalPrice from Bill WHERE status = 'Finished' AND enabled = true AND DAY(date) = :day AND MONTH(date) = :month AND YEAR(date) = :year";
}
